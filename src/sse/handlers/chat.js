...
// Combine both: combo strategies AND capacity adapter
import { handleComboChat, handleFusionChat, resolveComboStrategy, buildComboCtx, detectRequiredCapabilities } from "open-sse/services/combo.js";
...
export async function handleChat(request, clientRawRequest = null) {
  ...
  if (comboModels) {
    // Use combo strategy first (from your commit), then augment with capacity adapter
    const { strategy: comboStrategy, comboCfg } = resolveComboStrategy(settings, modelStr);
    
    // Augment with capacity adapter support
    const augmentedModels = augmentModelsWithCapacityAdapter(comboModels, requiredCapabilities, settings);
    const adapterAdded = augmentedModels.filter((m) => !comboModels.includes(m));

    if (comboStrategy === "fusion") {
      log.info("CHAT", `Combo "${modelStr}" with ${augmentedModels.length} models (strategy: fusion)`);
    ...
    const comboCtx = buildComboCtx({ models: augmentedModels, comboCfg, body });
